...
def grid(rows_mult=None, buckets=None, **rows):
    """Cells are one value per bucket of ten blocks."""
...
check_grid("one bucket off", grid(qkv=[0, 1, 1, 1, 1]), "0-9.qkv: 0")
check_grid("adjacent equal buckets merge",
           grid(fc1=[0, 0, 1, 1, 1]), "0-19.fc1: 0")
check_grid("whole row off", grid(out=[0] * 5), "0-49.out: 0")
check_grid("two runs in one row",
           grid(fc2=[0, 1, 1, 1, 0]), "0-9.fc2: 0" + chr(10) + "40-49.fc2: 0")
check_grid("fractional values survive", grid(qkv=[0.5] * 5), "0-49.qkv: 0.5")
check_grid("multiple rows keep row order",
           grid(qkv=[0] * 5, fc1=[0] * 5), "0-49.qkv: 0" + chr(10) + "0-49.fc1: 0")
...
check_grid("a muted cell beats a softer row",
           grid(rows_mult={"out": 0.5}, out=[0, 1, 1, 1, 1]),
           "0-9.out: 0" + chr(10) + "10-49.out: 0.5")
...
check_grid("missing rows/buckets keys tolerated",
           _json.dumps({r: [1] * h3.NUM_BUCKETS for r in h3.GRID_ROWS}), "")
check_grid("garbage multipliers fall back to 1",
           grid(rows_mult={"fc1": "nope"}, buckets=["x"] * 5), "")

# old 50-value state folds down, keeping the most restrictive value per bucket
_old = _json.dumps({"qkv": [0] * 10 + [1] * 40, "out": [1] * 50,
                    "fc1": [1] * 50, "fc2": [1] * 50})
check_grid("legacy per-block state migrates", _old, "0-9.qkv: 0")
...
rules = h3.parse_spec(h3.spec_from_grid(grid(fc1=[0, 1, 1, 1, 1], fc2=[0, 1, 1, 1, 1])))
...
rules = h3.parse_spec(h3.build_spec(1.0, grid(qkv=[0] * 5), "0-9.qkv: 1.0"))
assert h3.resolve_weight(rules, "block", 5, "attn.qkv_proj") == 1.0
